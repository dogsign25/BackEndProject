...
public class SpotifyService {
    ...
    /**
     * 아티스트명 기반으로 비슷한 곡 검색 (recommendations API 대신 search 사용)
     * 
     * @param accessToken Spotify 액세스 토큰
     * @param artistName 아티스트 이름
     * @param limit 검색할 곡 개수
     * @return 추천 트랙 리스트
     */
    public List<TrackDTO> getRecommendationsByArtistName(String accessToken, String artistName, int limit) {
        List<TrackDTO> list = new ArrayList<>();
        ...
        try {
            ...
            int responseCode = conn.getResponseCode();
            if (responseCode != 200) {
                System.err.println("[Spotify API] Artist-based search 요청 실패. Response Code: " + responseCode);
                return list;
            }
            ...
            System.err.println("[Spotify API] Artist-based search 중 Exception:");
            e.printStackTrace();
        }
